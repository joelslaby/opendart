...
import json
import os
from dart_engine.params_aroundTheWorld import Hit, ATWGame, Player
from dart_engine.helpers_aroundTheWorld import get_game_marks_complete
...
class DartsApp:
    ...
    def __init__(self, root):
        ...
        tk.Label(btn_frame4, text="Player: ", font=("Arial",20)).pack(side=tk.LEFT, padx=5)
    ...
    def save(self):
        ...
        self.filename = f"ATW_{self.game.players[0].name}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json"
    ...
    def draw_scoreboard(self):
        ...
        c.create_line(size_x/2, 0, size_x/2, 600, fill="white", width=2)
        ...
        c.create_text(
            size_x*1/4,
            y+60,
            text=str(ATW_ORDER[self.game.players[0].current_number_idx]),
            font=("Arial",60,"bold")
        )
    ...
    def draw_infoboard(self):
        ...
        current_dart_number = len(hist) % 3

        p0_hit_number = 0
        p0_current_hits = []
        p0_hits = []
        current_name = player_list[0].name
        p0_current_hits_number = 0
        for hh,hit in enumerate(hist):
            if p0_hit_number > 2:
                continue
            if p0_current_hits_number > current_dart_number-1:
